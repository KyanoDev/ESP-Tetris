...
export function useGameLogic() {
    ...
    const generateNewPiece = useCallback(() => {
        ...
        const newPiece: ActivePiece = {
            ...
            position: {
                x: Math.floor(BOARD_WIDTH / 2) - Math.floor(newTetromino.shapes[0][0].length / 2),
                y: 0,
            },
            rotation: 0,
        }
    ...
    const lockPiece = useCallback(() => {
        ...
        if (completedLines > 0) {
            ...
            if (newLevel > level) {
                ...
                const newSpeed = Math.max(INITIAL_DROP_SPEED - (newLevel - 1) * SPEED_INCREASE_PER_LEVEL, MIN_DROP_SPEED)
        ...
        generateNewPiece()
    }, [activePiece, getCurrentShape, generateNewPiece, level, lines, lockedPieces, score])
    ...
    const rotate = useCallback(() => {
        ...
        const newRotation = (activePiece.rotation + 1) % activePiece.tetromino.shapes.length
    ...
    const startGame = useCallback(() => {
        ...
        const newPiece = {
            ...
            position: {
                x: Math.floor(BOARD_WIDTH / 2) - Math.floor(initialPiece.shapes[0][0].length / 2),
                y: 0,
            },
            rotation: 0,
        }
